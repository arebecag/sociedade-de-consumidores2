...
import { Eye, EyeOff, Loader2, CheckCircle, AlertCircle, Shield, Zap, Users } from 'lucide-react';
      ...
      <div className="min-h-screen bg-zinc-950 flex items-center justify-center p-4">
        <div className="w-full max-w-md">
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-black text-white mb-1">Cadastro realizado!</h1>
            <p className="text-orange-400 font-semibold text-lg">Bem-vindo(a), {nomeRegistrado}! 🎉</p>
          </div>
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 mb-6 space-y-4">
            <p className="text-gray-400 text-sm font-semibold uppercase tracking-wider">Próximos passos</p>
            {[
              ["1", "Verifique seu e-mail para confirmar sua conta"],
              ["2", "Faça login com seu e-mail e senha criados"],
              ["3", "Complete seu perfil e realize sua primeira compra para ser ativado"],
            ].map(([n, t]) => (
              <div key={n} className="flex items-start gap-3">
                <span className="w-6 h-6 rounded-full bg-orange-500 text-white text-xs font-bold flex items-center justify-center flex-shrink-0 mt-0.5">{n}</span>
                <p className="text-gray-300 text-sm leading-relaxed">{t}</p>
              </div>
            ))}
          </div>
          <Button onClick={() => navigate(createPageUrl('LoginCustom'))} className="w-full bg-orange-500 hover:bg-orange-600 text-white font-semibold h-12 text-base">
            Ir para o Login →
          </Button>
          <p className="text-gray-600 text-xs text-center mt-4">⚠️ Não tente se cadastrar novamente — sua conta já foi criada!</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-zinc-950 flex items-center justify-center p-4">
        <div className="text-center space-y-4 max-w-sm">
          ...
          <p className="text-gray-400 text-sm">O código de indicação não foi encontrado.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="hidden lg:flex lg:w-[45%] bg-gradient-to-br from-zinc-900 via-zinc-950 to-black flex-col justify-between p-12 border-r border-zinc-800">
        <div>
          ...
          <h1 className="text-4xl font-black text-white leading-tight mb-4">
            Comece a<br/><span className="text-orange-500">receber bônus</span><br/>hoje mesmo.
          </h1>
          <p className="text-gray-400 text-lg leading-relaxed">Crie sua conta em menos de 2 minutos e comece a construir sua rede.</p>
        </div>
        <div className="space-y-4">
          {[
            { icon: Zap, title: "Rápido e Gratuito", desc: "Cadastro em minutos" },
            { icon: Users, title: "Rede 3x3", desc: "Sistema inteligente" },
            { icon: Shield, title: "100% Seguro", desc: "Seus dados protegidos" },
      ...
      <div className="flex-1 flex flex-col justify-center px-4 sm:px-6 py-8">
        <div className="w-full max-w-md mx-auto">
          ...
          <p className="text-gray-400 text-sm mb-6">
            Já tem uma conta?{" "}
            <button type="button" onClick={() => navigate(createPageUrl('LoginCustom'))} className="text-orange-400 hover:text-orange-300 font-medium">
              Faça login →
            </button>
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <Input value={formData.full_name} onChange={e => handleChange('full_name', e.target.value)}
                className="bg-zinc-900 border-zinc-700 text-white focus:border-orange-500 h-11" placeholder="Seu nome completo" disabled={loading} />
            ...
            <div>
              ...
              <Input type="email" value={formData.email} onChange={e => handleChange('email', e.target.value)}
                className="bg-zinc-900 border-zinc-700 text-white focus:border-orange-500 h-11" placeholder="seu@email.com" disabled={loading} />
            ...
            <div>
              ...
              <div className="relative">
                <Input type={showPassword ? 'text' : 'password'} value={formData.password} onChange={e => handleChange('password', e.target.value)}
                  className="bg-zinc-900 border-zinc-700 text-white focus:border-orange-500 h-11 pr-10" placeholder="Mín. 8 chars, maiúscula e número" disabled={loading} />
            ...
            <div>
              <Label className="text-gray-300 text-sm mb-1.5 block">Repetir Senha *</Label>
              <div className="relative">
                <Input type={showConfirmPassword ? 'text' : 'password'} value={formData.confirmPassword} onChange={e => handleChange('confirmPassword', e.target.value)}
                  className="bg-zinc-900 border-zinc-700 text-white focus:border-orange-500 h-11 pr-10" placeholder="Digite a senha novamente" disabled={loading} />
