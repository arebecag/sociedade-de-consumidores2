...
export default function AdminNetwork() {
  ...
  const loadNetworkData = async (partnerId) => {
    try {
      ...
      const directNetwork = network.filter((p) =>
        networkRelations.find(
          (n) => n.referred_id === p.id && n.relation_type === "direct",
        ),
      );

      const indirectNetwork = network.filter((p) =>
        networkRelations.find(
          (n) => n.referred_id === p.id && n.relation_type === "indirect",
        ),
      );
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-white">Auditoria de Clientes</h1>
        <p className="text-gray-400">
          Visualize os clientes vinculados a qualquer parceiro
        </p>
      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <p className="text-white">
                  {selectedPartner.graduation?.replace("_", " ")}
                </p>
              </div>
              <div>
                <p className="text-gray-400 text-sm">Bônus Total</p>
                <p className="text-white">
                  {(selectedPartner.total_bonus_generated || 0).toFixed(2)}
                </p>
              </div>
            </div>
          </CardContent>
        ...
        <>
              ...
              <CardContent>
                <div className="flex items-center gap-4 p-4 bg-zinc-900 rounded-lg">
                  ...
                  <div className="flex-1">
                    <p className="text-white font-semibold">
                      {networkData.referrer.display_name ||
                        networkData.referrer.full_name}
                    </p>
                    <p className="text-gray-400 text-sm">
                      {networkData.referrer.created_by}
                    </p>
                  </div>
                  <Badge
                    className={
                      networkData.referrer.status === "ativo"
                        ? "bg-green-500/20 text-green-500"
                        : networkData.referrer.status === "pendente"
                          ? "bg-yellow-500/20 text-yellow-500"
                          : "bg-red-500/20 text-red-500"
                    }
                  >
          ...
          {networkData.indirectNetwork.length > 0 && (
            <Card className="bg-zinc-950 border-orange-500/20">
              <CardHeader>
                <CardTitle className="text-white">
                  Clientes Indiretos (Desempenho 2)
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-2">
