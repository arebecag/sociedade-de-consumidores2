...
export default function Register() {
  ...
  const generateUniqueCode = async () => {
    ...
    while (!isUnique) {
      ...
      const existing = await base44.entities.Partner.filter({
        unique_code: code,
      });
  ...
  const validateStep1 = () => {
    ...
    if (!formData.birth_date) newErrors.birth_date = "Obrigatória";
      ...
      newErrors.birth_date = "Mínimo 15 anos";
    if (!formData.gender) newErrors.gender = "Selecione";
    if (!formData.phone || formData.phone.replace(/\D/g, "").length < 11)
      newErrors.phone = "DDD + número";
    setErrors(newErrors);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const partnerData = {
        ...
        status: "pendente",
        pending_reasons: [
          "Falta da primeira compra",
          "Falta de informações no cadastro",
        ],
      ...
      if (!res.data?.partner?.id)
        throw new Error(
          "Falha ao criar perfil: " + (res.data?.error || "Tente novamente."),
        );
      ...
      const msg =
        xe?.response?.data?.message ||
        xe?.response?.data?.error ||
        xe?.message ||
        "";
      const isEmailDup =
      ...
      <div className="min-h-screen bg-[#1a1a1a] flex items-center justify-center p-4">
        ...
        >
          ...
          <p className="text-zinc-400 text-sm">
            O código de indicação não foi encontrado. Verifique o link e tente
            novamente.
          </p>
        </motion.div>
      </div>
        ...
        <div className="relative z-10 flex flex-col h-full p-12">
          ...
          >
            ...
            <p className="text-zinc-400 text-base leading-relaxed max-w-xs">
              Receba bônus semanais pelas compras dos seus clientes. Pague
              boletos, contas e muito mais com seus bônus.
            </p>
          </motion.div>
          ...
          >
            {[
              ...
              <div
                key={label}
                className={`p-4 rounded-2xl border ${bg} backdrop-blur-sm`}
              >
                  ...
                  <div className="flex items-center gap-2 mb-6">
                    {[1, 2].map((s) => (
                      <React.Fragment key={s}>
                        ...
                        {s < 2 && (
                          <div
                            className={`flex-1 h-0.5 rounded-full transition-all duration-500 ${step > s ? "bg-green-500" : "bg-zinc-800"}`}
                          />
                        )}
                      </React.Fragment>
                    ))}
                        ...
                        >
                          ...
                          <div>
                            ...
                            <Input
                              ...
                              className="bg-zinc-800 border-zinc-700 text-white focus:border-orange-500 rounded-xl h-12 placeholder:text-zinc-600"
                          ...
                          <div>
                            ...
                            <div className="relative">
                              <Input
                                ...
                                className="bg-zinc-800 border-zinc-700 text-white focus:border-orange-500 rounded-xl h-12 pr-12 placeholder:text-zinc-600"
                            ...
                            {formData.password && (
                              <div className="mt-2 space-y-1">
                                ...
                                {passwordStrength.message && (
                                  <p
                                    className={`text-xs ${strengthTextColors[passwordStrength.score]}`}
                                  >
                                    {passwordStrength.message}
                                  </p>
                                )}
                            ...
                            {[
                              {
                                ...
                                label: "Li e aceito o",
                                linkLabel: "Contrato de Serviços",
                                onClick: () => setTermsOpen(true),
                                error: errors.accepted_terms,
                              },
                              {
                                id: "rules",
                                field: "accepted_rules",
                                label: "Li e aceito o",
                                linkLabel: "Regimento Interno",
                              ...
                              ({
                                ...
                                <div key={id}>
                                  <div className="flex items-start gap-3">
                                    ...
                                    <label
                                      htmlFor={id}
                                      className="text-zinc-400 text-sm cursor-pointer leading-snug"
                                    >
                                      {label}{" "}
                                      <button
                                        type="button"
                                        onClick={onClick}
                                        className="text-orange-400 hover:text-orange-300 font-semibold transition-colors underline underline-offset-2"
                                      >
                                        {linkLabel}
                                      </button>
                                    </label>
                                  ...
                                  {error && (
                                    <p className="text-red-400 text-xs pl-7 mt-1">
                                      {error}
                                    </p>
                                  )}
                                </div>
        ...
        <DialogContent className="bg-zinc-950 border-zinc-800 max-h-[80vh] overflow-y-auto">
          ...
          <div className="text-zinc-300 text-sm space-y-4">
            ...
            <p className="text-red-300">
              <strong>ATENÇÃO:</strong> A SOCIEDADE DE CONSUMIDORES NUNCA envia
              cobrança por PIX. A única forma de pagamento é por boleto gerado
              em nosso sistema.
            </p>
            <p className="text-yellow-300">
              <strong>ATENÇÃO:</strong> NUNCA GASTE SEU DINHEIRO fazendo compras
              exageradas. Use nosso sistema de GERAÇÃO de BÔNUS.
            </p>
            <p className="text-green-400 font-bold">
              GARANTIA ABSOLUTA! Se não estiver satisfeito, devolvemos o seu
              dinheiro sem perguntas.
            </p>
            <p>
              <strong>CLÁUSULA 1 – DO OBJETO:</strong> Uma PLATAFORMA ONLINE que
              administra um sistema inteligente de SOCIEDADE DE CONSUMIDORES e
              distribuição de BÔNUS entre seus associados.
            </p>
            <p>
              <strong>CLÁUSULA 2 – DEVERES DA GESTORA:</strong> Prestar
              atendimento, disponibilizar Escritório Virtual, manter o sistema e
              emitir notas fiscais.
            </p>
            <p>
              <strong>CLÁUSULA 3 – DEVERES DO CLIENTE:</strong> Usar o site
              pessoal para divulgação, efetuar compras mensais, ser maior de 15
              anos e não ostentar em redes sociais.
            </p>
            <p>
              <strong>CLÁUSULA 4 – DO FÓRUM:</strong> Câmara de Intermediação da
      ...
      <Dialog open={rulesOpen} onOpenChange={setRulesOpen}>
        <DialogContent className="bg-zinc-950 border-zinc-800 max-h-[80vh] overflow-y-auto">
          ...
          <div className="text-zinc-300 text-sm space-y-4">
            ...
            <p className="text-yellow-300">
              <strong>ATENÇÃO:</strong> NUNCA GASTE SEU DINHEIRO fazendo compras
              exageradas. FAÇA SOMENTE AS COMPRAS MÍNIMAS MENSAIS.
            </p>
            <p className="text-green-400 font-bold">GARANTIA ABSOLUTA!</p>
            <p>
              <strong>REGIMENTO 2 – PAGAMENTOS:</strong> Nosso sistema permite
              pagar seus boletos com seus BÔNUS através de PIX ou código de
              barras.
            </p>
            <p>
              <strong>REGIMENTO 5 – STATUS:</strong> ATIVO (em dia), PENDENTE
              (pendências), EXCLUÍDO (fraudes).
            </p>
            <p>
              <strong>REGIMENTO 11 – DEPÓSITOS:</strong> Valor mínimo R$ 30,00.
              Depositado automaticamente toda segunda-feira.
            </p>
            <p>
              <strong>REGIMENTO 12 – GERAÇÃO DE BÔNUS:</strong> Até 40% do valor
              da compra dos seus clientes. 50% para saque e 50% para compras.
            </p>
            <p>
              <strong>REGIMENTO 14 – GRADUAÇÃO:</strong> LÍDER (60), ESTRELA
              (120), BRONZE (240), PRATA (480), OURO (960) clientes ativos.
            </p>
          </div>
        </DialogContent>
      </Dialog>
