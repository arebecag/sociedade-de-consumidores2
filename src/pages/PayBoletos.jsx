...
export default function PayBoletos() {
  ...
  const loadData = async () => {
    try {
      ...
      const userBoletos = await base44.entities.Boleto.filter({
  ...
  const handleSubmit = async () => {
    ...
    if (!validateDueDate(formData.due_date)) {
      toast.error(
        "O boleto deve ter vencimento com no mínimo 7 dias de antecedência",
      );
      return;
    }
    ...
    if (parseFloat(formData.amount) > (partner?.bonus_for_purchases || 0)) {
  ...
  const StatusPill = ({ status }) => {
    ...
    return (
      <span
        className={`flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold border ${cls}`}
      >
  ...
  const BoletoList = ({ list }) => {
    if (list.length === 0)
      return <EmptyState icon={FileText} message="Nenhum boleto encontrado." />;
    return (
      <motion.div
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div className="space-y-1">
                ...
                <p className="text-zinc-500 text-xs">
                  Enviado:{" "}
                  {new Date(
                    b.submitted_date || b.created_date,
                  ).toLocaleDateString("pt-BR")}
                </p>
                {b.paid_date && (
                  <p className="text-green-400 text-xs">
                    Pago em: {new Date(b.paid_date).toLocaleDateString("pt-BR")}
                  </p>
                )}
              </div>
              <div className="flex items-center gap-3">
                ...
                <div className="flex gap-2">
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => openExternalLink(b.file_url)}
                    className="border-orange-500/40 text-orange-400 hover:bg-orange-500/10 rounded-xl gap-1 text-xs"
                  >
                  ...
                  {b.receipt_url && (
                    <Button
                      size="sm"
                      className="bg-green-600 hover:bg-green-700 rounded-xl gap-1 text-xs"
                      onClick={() => openExternalLink(b.receipt_url)}
                    >
  ...
  return (
    <AnimatedPage>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 w-full sm:w-auto">
            <div className="px-4 py-2 rounded-xl bg-zinc-900 border border-white/[0.05]">
              ...
              <p className="text-orange-400 font-bold text-sm">
                {fmtR(partner?.bonus_for_purchases)}
              </p>
            </div>
            <Button
              onClick={() => {
                if (isBlocked) {
                  toast.error(
                    "Para usar todos os recursos, é necessário que você esteja ativo.",
                  );
                } else {
                  setUploadDialogOpen(true);
                }
              }}
              className="bg-orange-500 hover:bg-orange-600 rounded-xl gap-2 w-full sm:w-auto"
            >
        ...
        <div className="p-4 rounded-2xl bg-red-500/5 border border-red-500/20 flex items-start gap-3">
          ...
          <div>
            <p className="text-red-300 font-bold text-sm">
              ATENÇÃO: NUNCA CAIA EM GOLPES!
            </p>
            <p className="text-zinc-400 text-sm mt-0.5">
              A SOCIEDADE DE CONSUMIDORES NUNCA envia cobrança por PIX. A única
              forma de pagamento é por boleto gerado em nosso sistema.
            </p>
          </div>
        </div>
      </AnimatedItem>
        ...
        <AnimatedItem>
          <div className="p-4 rounded-2xl bg-yellow-500/5 border border-yellow-500/20 flex items-start gap-3">
            ...
            <p className="text-zinc-300 text-sm">
              <strong>Envio de boletos temporariamente bloqueado.</strong> Para
              usar todos os recursos, é necessário que você esteja ativo.
            </p>
          </div>
        </AnimatedItem>
      )}
      ...
      <AnimatedItem>
        <Tabs defaultValue="pending">
          <TabsList className="bg-zinc-900 border border-white/[0.05] mb-6 w-full sm:w-auto">
            <TabsTrigger
              value="pending"
              className="data-[state=active]:bg-orange-500 flex-1 sm:flex-none text-xs gap-1"
            >
          ...
          <TabsContent value="pending">
            <BoletoList list={pendingBoletos} />
          </TabsContent>
          <TabsContent value="paid">
            <BoletoList list={paidBoletos} />
          </TabsContent>
          <TabsContent value="all">
            <BoletoList list={boletos} />
          </TabsContent>
        </Tabs>
      </AnimatedItem>
      ...
      <Dialog open={uploadDialogOpen} onOpenChange={setUploadDialogOpen}>
        <DialogContent className="bg-zinc-950 border-zinc-800 w-[calc(100vw-2rem)] sm:max-w-4xl max-h-[90vh] overflow-y-auto p-4 sm:p-6">
          <DialogHeader>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
                  ...
                  <label htmlFor="boleto-file" className="cursor-pointer">
                    {selectedFile ? (
                      ...
                      <>
                        ...
                        <p className="text-zinc-600 text-xs">PDF, PNG, JPG</p>
                      </>
                    )}
                  </label>
                </div>
              ...
              {[
                {
                  label: "O boleto tem código de barras?",
                  field: "has_barcode",
                },
                ...
                <div key={field} className="space-y-2">
                  ...
                  <RadioGroup
                    ...
                    onValueChange={(v) =>
                      setFormData({ ...formData, [field]: v === "true" })
                    }
              ...
              {needsDetails.includes(formData.product_type) && (
                <div className="space-y-2">
                  <Label className="text-zinc-300 text-sm font-semibold">
                    {formData.product_type === "loja_fisica"
                      ? "Nome da loja e do produto"
                      : formData.product_type === "servico"
                        ? "Nome do prestador e tipo de serviço"
                        : "Especifique os detalhes"}
                  </Label>
                  <Textarea
            ...
            <div className="space-y-4">
                  ...
                  {[
                    ...
                    <div key={v} className="flex items-center gap-2">
                      <RadioGroupItem
                        ...
                        className="border-orange-500 text-orange-500"
                      />
                      <Label htmlFor={`pay-${v}`} className="text-zinc-300">
                        {l}
                      </Label>
                    </div>
                  ))}
              ...
              <div className="space-y-2">
                ...
                />
                <p className="text-zinc-600 text-xs">
                  Saldo disponível: {fmtR(partner?.bonus_for_purchases)}
                </p>
                {formData.amount && parseFloat(formData.amount) > 0 && (
                  <div className="p-3 rounded-xl bg-orange-500/5 border border-orange-500/20">
                    <p className="text-orange-400 text-xs font-semibold">
                      Conversão de Bônus (+346%)
                    </p>
                    <p className="text-orange-300 font-bold">
                      {fmtR(parseFloat(formData.amount) * 4.46)} em bônus
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="p-3 rounded-xl bg-zinc-800/60 border border-zinc-700">
                ...
                <p className="text-zinc-500 text-xs">
                  Para pagar um boleto, você precisa de 346% a mais do valor em
                  bônus.
                  <br />
                  Ex: Boleto de R$ 100,00 = R$ 446,00 em bônus.
                </p>
              </div>
            </div>
          </div>
