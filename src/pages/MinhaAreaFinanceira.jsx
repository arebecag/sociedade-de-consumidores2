          ...
          <Card className="bg-zinc-950 border-orange-500/20">
            <CardContent className="p-5">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                  <p className="text-gray-400 text-sm">Saldo disponível para saque</p>
                  <p className="text-3xl font-bold text-white">{formatCurrency(saldoSaque)}</p>
                  {!partner?.pix_key && (
                    <p className="text-yellow-400 text-xs mt-1">⚠ Cadastre uma chave PIX no perfil para solicitar saques.</p>
                  )}
                  {saldoSaque < VALOR_MINIMO_SAQUE && (
                    <p className="text-gray-500 text-xs mt-1">Mínimo para saque: {formatCurrency(VALOR_MINIMO_SAQUE)}</p>
                  )}
                </div>
                <Button
                  onClick={solicitarSaque}
                  disabled={solicitandoSaque || saldoSaque < VALOR_MINIMO_SAQUE || !partner?.pix_key}
