...
export default function MinhaAreaFinanceira() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [fins, sqs] = await Promise.all([
        base44.entities.Financeiro.filter(
          { userId: currentPartner.id },
          "-created_date",
          50,
        ),
        base44.entities.Saques.filter(
          { userId: currentPartner.id },
          "-created_date",
          20,
        ),
  ...
  const gerarBoleto = async () => {
    ...
    try {
      ...
      if (res.data?.success) {
        const invoiceUrl =
          res.data?.boleto?.invoiceUrl || res.data?.boleto?.bankSlipUrl;
        toast.success(
          res.data.reutilizado
            ? "Usando cobrança existente!"
            : "Boleto gerado!",
        );
        loadData();
        if (invoiceUrl) openExternalLink(invoiceUrl);
  ...
  const solicitarSaque = async () => {
    ...
    if (valor > saldo) {
      toast.error(`Valor excede o saldo (${fmtR(saldo)})`);
      return;
    }
    if (!cobranças.some((c) => ["CONFIRMED", "RECEIVED"].includes(c.status))) {
      toast.error("Necessário ter um pagamento confirmado.");
      return;
    }
    if (saques.some((s) => s.status === "PENDENTE")) {
  ...
  const cobrancaAtiva = cobranças.find((c) =>
    ["PENDING", "CONFIRMED", "RECEIVED"].includes(c.status),
  );
  const proximaCobranca = cobranças.find((c) => c.status === "PENDING");
  const totalPago = cobranças
    .filter((c) => ["CONFIRMED", "RECEIVED"].includes(c.status))
    .reduce((s, c) => s + (c.valor || 0), 0);
  const totalBonus = cobranças
    .filter((c) => c.bonusLiberado)
    .reduce((s, c) => s + (c.valorBonus || 0), 0);
  ...
  const summaryCards = [
    ...
    },
    {
      ...
      value: proximaCobranca ? fmtDate(proximaCobranca.dataVencimento) : "—",
      icon: Calendar,
      color: "text-blue-400",
      bg: "bg-blue-500/10",
    },
      ...
      <PageHeader
        ...
        action={
          <Button
            onClick={loadData}
            variant="outline"
            className="border-zinc-700 text-zinc-400 hover:text-white rounded-xl gap-2"
          >
                ...
                {!partner?.cpf && (
                  <div className="p-4 rounded-2xl bg-yellow-500/5 border border-yellow-500/20 flex items-center gap-3">
                    ...
                    <p className="text-zinc-300 text-sm">
                      Para gerar boletos, cadastre seu CPF no perfil primeiro.
                    </p>
                  </div>
                )}
                {cobrancaAtiva ? (
                  <div className="p-5 rounded-2xl bg-zinc-900/60 border border-white/[0.05] space-y-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {(() => {
                        const st = STATUS[cobrancaAtiva.status];
                        return st ? (
                          <span
                            className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold border ${st.cls}`}
                          >
                      ...
                      {cobrancaAtiva.bonusLiberado && (
                        <span className="px-2.5 py-1 rounded-full text-xs font-bold border bg-orange-500/10 text-orange-400 border-orange-500/20">
                          🎉 Bônus Liberado
                        </span>
                      )}
                    </div>
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                      {[
                        {
                          l: "Valor",
                          v: fmtR(cobrancaAtiva.valor),
                          c: "text-white",
                        },
                        {
                          l: "Vencimento",
                          v: fmtDate(cobrancaAtiva.dataVencimento),
                          c: "text-zinc-200",
                        },
                        cobrancaAtiva.dataPagamento && {
                          l: "Pago em",
                          v: fmtDate(cobrancaAtiva.dataPagamento),
                          c: "text-green-400",
                        },
                        cobrancaAtiva.valorBonus > 0 && {
                          l: "Bônus Gerado",
                          v: fmtR(cobrancaAtiva.valorBonus),
                          c: "text-orange-400",
                        },
                      ]
                        .filter(Boolean)
                        .map(({ l, v, c }) => (
                          <div
                            key={l}
                            className="p-3 rounded-xl bg-zinc-800/60"
                          >
                    ...
                    {cobrancaAtiva.status === "PENDING" && (
                      <div className="flex flex-col sm:flex-row gap-2">
                        ...
                        {cobrancaAtiva.bankSlipUrl && (
                          <a
                            href={cobrancaAtiva.bankSlipUrl}
                            target="_blank"
                            rel="noreferrer"
                          >
                            <Button
                              variant="outline"
                              className="border-zinc-700 text-zinc-300 rounded-xl gap-2"
                            >
                  ...
                  <div className="rounded-2xl bg-zinc-900/60 border border-white/[0.05] p-4">
                    <EmptyState
                      ...
                      sub="Gere um boleto para ativar seu acesso."
                    />
                          ...
                          <div className="flex gap-2 shrink-0">
                            {c.invoiceUrl && (
                              ...
                              >
                                <Button
                                  size="sm"
                                  className="bg-orange-500 hover:bg-orange-600 rounded-xl gap-1 text-xs"
                                >
                            ...
                            {c.bankSlipUrl && (
                              ...
                              >
                                <Button
                                  size="sm"
                                  variant="outline"
                                  className="border-zinc-700 text-zinc-400 rounded-xl gap-1 text-xs"
                                >
            ...
            {aba === "saques" && (
              <div className="space-y-4">
                  ...
                  {temSaquePendente ? (
                    ...
                    <div className="space-y-3 pt-3 border-t border-white/[0.05]">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                          ...
                          <Input
                            ...
                            placeholder={`Máx: ${fmtR(saldoSaque)}`}
                            max={saldoSaque}
                            min={0.01}
                          />
                        </div>
                      ...
                      <Button
                        ...
                        disabled={
                          solicitandoSaque || !valorSaque || !pixKeySaque
                        }
                      ...
                      <p className="text-zinc-600 text-xs">
                        O admin irá processar sua solicitação em breve.
                      </p>
                    </div>
                  ) : (
                ...
                {saques.length === 0 ? (
                  <EmptyState
                    icon={ArrowDownCircle}
                    message="Nenhuma solicitação de saque ainda."
                  />
                ) : (
                  saques.map((s) => {
                    ...
                    return (
                      <div
                        key={s.id}
                        className="p-4 rounded-2xl bg-zinc-900/60 border border-white/[0.05] flex items-center justify-between"
                      >
                        <div className="space-y-1">
                          <span
                            className={`px-2.5 py-1 rounded-full text-xs font-bold border ${st.cls}`}
                          >
                            {st.label}
                          </span>
                          <p className="text-white font-bold text-lg">
                          ...
                          <p className="text-zinc-500 text-xs">
                            Solicitado: {fmtDT(s.dataSolicitacao)}
                          </p>
                          {s.dataPagamento && (
                            <p className="text-green-400 text-xs">
                              Pago: {fmtDT(s.dataPagamento)}
                            </p>
                          )}
                          {s.observacao && (
