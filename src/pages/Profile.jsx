      ...
      <div className="space-y-4">
        <div className="p-3 rounded-xl bg-orange-500/5 border border-orange-500/20 flex items-start gap-2">
          ...
          <p className="text-zinc-300 text-sm">
            Você receberá um código de confirmação no novo email. Após
            confirmar, será necessário fazer login novamente.
          </p>
        </div>
        <div className="space-y-2">
          ...
          <Input
            value={currentEmail}
            disabled
            className="bg-zinc-900 border-zinc-700 text-zinc-400 rounded-xl"
          />
        </div>
        <div className="space-y-2">
        ...
        <DialogFooter>
          ...
          >
            {loading && <Loader2 className="w-4 h-4 animate-spin mr-2" />}Enviar
            Código
          </Button>
        </DialogFooter>
      </div>
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <Input
          ...
          className="bg-zinc-900 border-zinc-700 text-white text-center text-2xl tracking-widest rounded-xl"
          placeholder="000000"
        />
...
const fieldClass =
  "bg-zinc-800/60 border-zinc-700 text-white focus:border-orange-500 rounded-xl";
const labelClass =
  "text-zinc-400 text-xs font-semibold uppercase tracking-wider mb-1.5 block";
...
export default function Profile() {
  ...
  const populateForm = (p, user) => {
    ...
    setFormData({
      ...
      notification_third_party_purchases:
        p.notification_third_party_purchases ?? true,
  ...
  const handleSave = async () => {
    if (!partner) {
      toast.error("Erro: dados do parceiro não carregados.");
      return;
    }
    setSaving(true);
    try {
      ...
      if (allComplete && partner.pending_reasons?.length > 0) {
        const newReasons = partner.pending_reasons.filter(
          (r) => r !== "Falta de informações no cadastro",
        );
      ...
      {partner?.status === "pendente" && (
        <AnimatedItem>
          <div className="p-5 rounded-2xl bg-yellow-500/5 border border-yellow-500/20">
            ...
            <div className="space-y-2">
              <ChecklistItem
                ok={formData.cpf && formData.cpf.length === 11}
                label="CPF completo"
              />
              <ChecklistItem
                ok={!!(formData.pix_key && formData.pix_key_type)}
              ...
              />
              <ChecklistItem
                ok={partner.first_purchase_done}
                label="Primeira compra realizada"
              />
            </div>
          </div>
        </AnimatedItem>
            ...
            <TabsList className="bg-zinc-900 border border-white/[0.05] w-max">
              {[
                ...
                <TabsTrigger
                  key={value}
                  value={value}
                  className="data-[state=active]:bg-orange-500 text-xs gap-1"
                >
            ...
            <div className="p-5 rounded-2xl bg-zinc-900/60 border border-white/[0.05] space-y-5">
              ...
              {partner && (
                <div className="flex items-center gap-3">
                  ...
                  <span
                    className={`px-3 py-1 rounded-full text-xs font-bold ${partner.status === "ativo" ? "bg-green-500/10 text-green-400" : partner.status === "pendente" ? "bg-yellow-500/10 text-yellow-400" : "bg-red-500/10 text-red-400"}`}
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  ...
                  <div key={field}>
                    ...
                    />
                    {field === "cpf" && formData.cpf.length !== 11 && (
                  ...
                  <div className="flex gap-2">
                    ...
                    <Dialog>
                      <DialogTrigger asChild>
                        <Button
                          variant="outline"
                          className="border-orange-500/40 text-orange-400 hover:bg-orange-500/10 rounded-xl shrink-0"
                        >
                          Alterar
                        </Button>
                      </DialogTrigger>
                  ...
                  {formData.pix_key_type === "pix" && (
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          handleChange("pix_key", e.target.value)
                        }
                        className={fieldClass}
                  ...
                  {formData.pix_key_type === "ted" && (
                    <div className="flex items-center">
                      <p className="text-zinc-400 text-sm">
                        Pagamento via TED usando os dados bancários acima.
                      </p>
                    </div>
                  )}
                </div>
            ...
            <div className="p-5 rounded-2xl bg-zinc-900/60 border border-white/[0.05] space-y-5">
              ...
              <div className="space-y-3">
                {[
                  {
                    label: "E-mail",
                    desc: "Notificações por e-mail (obrigatório)",
                  ...
                  {
                    label: "WhatsApp",
                    desc: "Notificações por WhatsApp",
                    field: "notification_whatsapp",
                  },
                ].map(({ label, desc, field, disabled, checked }) => (
                  <div
                    key={field}
                    className="flex items-center justify-between p-4 rounded-xl bg-zinc-800/60"
                  >
                    <div>
                    ...
                    <Switch
                      checked={disabled ? true : formData[field]}
                      onCheckedChange={
                        disabled ? undefined : (v) => handleChange(field, v)
                      }
                      disabled={disabled}
              ...
              <div className="pt-4 border-t border-white/[0.05]">
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    handleChange("notification_frequency", v)
                  }
                >
              ...
              <div className="pt-4 border-t border-white/[0.05] space-y-3">
                {[
                  {
                    label: "Novo Cadastro",
                    desc: "Notificações de novos clientes",
                    field: "notification_new_signup",
                  },
                  {
                    label: "Compras Próprias",
                    desc: "Notificações sobre suas compras",
                    field: "notification_own_purchases",
                  },
                  {
                    label: "Compras da Rede",
                    desc: "Notificações de compras de clientes",
                    field: "notification_third_party_purchases",
                  },
            ...
            <div className="p-5 rounded-2xl bg-zinc-900/60 border border-white/[0.05] space-y-5">
              <div>
                ...
                <p className="text-zinc-500 text-sm mt-1">
                  Em caso de falecimento, seus bônus e clientes vinculados serão
                  transferidos para o sucessor indicado.
                </p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  ...
                  <div key={field}>
                    ...
                    <Input
                      ...
                      value={
                        field === "cpf"
                          ? formatCPF(formData.successor.cpf || "")
                          : formData.successor[field]
                      }
                      onChange={(e) =>
      ...
      <AnimatedItem>
        <div className="p-5 rounded-2xl bg-red-500/5 border border-red-500/20">
          ...
          <ul className="text-zinc-400 text-sm space-y-1 mb-4 ml-4 list-disc">
            ...
            <li>
              O saldo para saque será depositado integralmente na sua conta
            </li>
          ...
          <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
            <DialogTrigger asChild>
              <Button
                variant="destructive"
                className="rounded-xl gap-2 bg-red-500 hover:bg-red-600"
              >
