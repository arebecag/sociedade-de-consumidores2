...
const quickActions = [
  {
    ...
    icon: ShoppingBag,
    page: "Store",
    desc: "Comprar produtos",
  },
  {
    label: "Meus Clientes",
    icon: Users,
    page: "Network",
    desc: "Acompanhar clientes",
  },
  {
    label: "Sacar Bônus",
    icon: Wallet,
...
export default function Dashboard() {
  ...
  const loadNetworkStats = async (p) => {
    ...
    try {
      ...
      if (allReferredIds.length > 0) {
        const allPartners = await base44.entities.Partner.list(null, 500);
        const myReferred = allPartners.filter((rp) =>
          allReferredIds.includes(rp.id),
        );
        setNetworkStats({
    ...
    return (
      <div className="text-center py-16">
        ...
        <p className="text-zinc-400">
          Acesse seu perfil para completar as informações.
        </p>
      </div>
    );
  }
  ...
  const bonusItems = [
    {
      label: "Total Gerado",
      value: fmtR(partner.total_bonus_generated),
      icon: TrendingUp,
    ...
    {
      label: "Para Saque",
      value: fmtR(partner.bonus_for_withdrawal),
      icon: Wallet,
    ...
    {
      label: "Para Compras",
      value: fmtR(partner.bonus_for_purchases),
      icon: ShoppingBag,
    ...
    },
    {
      ...
      value: fmtR(partner.total_withdrawn),
      icon: CreditCard,
      color: "text-blue-400",
      bg: "bg-blue-500/10",
    },
    {
      ...
      value: fmtR(partner.total_spent_purchases),
      icon: BarChart2,
      color: "text-pink-400",
      bg: "bg-pink-500/10",
    },
      ...
      <AnimatedItem>
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          <div className="min-w-0">
            ...
            <p className="text-zinc-600 text-sm mt-1 truncate">
              {partner.email}
            </p>
          </div>
          <div className="flex flex-wrap gap-2 shrink-0">
            ...
            <span
              className={`px-3 py-1.5 rounded-full text-xs font-bold border ${grad.bg} ${grad.color}`}
            >
              {grad.label}
            </span>
            {partner.unique_code && (
          ...
          <div className="p-4 rounded-2xl bg-yellow-500/5 border border-yellow-500/15 flex items-start gap-3">
            ...
            <div>
              <p className="text-yellow-300 font-semibold text-sm mb-1">
                Conta Pendente — ação necessária
              </p>
            ...
            {networkStats.direct === 0 && networkStats.indirect === 0 ? (
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  {
                    label: `Desempenho 1 (${networkStats.direct % 3}/3)`,
                    value: networkStats.direct,
                    color: "text-blue-400",
                  },
                  {
                    label: `Desempenho 2 (${networkStats.indirect % 9}/9)`,
                    value: networkStats.indirect,
                    color: "text-purple-400",
                  },
            ...
            {myReferrer ? (
              <div className="p-5 rounded-2xl bg-zinc-900/60 border border-white/[0.05] flex items-center gap-4">
                <div className="w-12 h-12 rounded-2xl bg-orange-500/10 border border-orange-500/20 flex items-center justify-center flex-shrink-0">
                  <span className="text-orange-400 text-xl font-bold">
                    {
                      (myReferrer.display_name ||
                        myReferrer.full_name ||
                        "?")[0]
                    }
                  </span>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-white font-semibold truncate">
                    {myReferrer.display_name || myReferrer.full_name}
                  </p>
                  <p className="text-zinc-500 text-xs font-mono mt-0.5">
                    #{myReferrer.unique_code}
                  </p>
                </div>
                <span
                  className={`px-2 py-1 rounded-full text-xs font-bold flex-shrink-0 ${
                    myReferrer.status === "ativo"
                      ? "bg-green-500/10 text-green-400"
                      : myReferrer.status === "pendente"
              ...
              <div className="p-5 rounded-2xl bg-zinc-900/60 border border-white/[0.05] flex items-center justify-center min-h-[80px]">
                <p className="text-zinc-500 text-sm">
                  Você é o primeiro da plataforma.
                </p>
              </div>
            )}
          </div>
