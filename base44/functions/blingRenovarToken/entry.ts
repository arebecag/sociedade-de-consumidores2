    ...
    if (!tokenResponse.ok) {
      ...
      await base44.asServiceRole.entities.IntegracaoBling.update(
        integracao.id,
        {
          ...
          ultimo_erro: `${tokenData.error_description || tokenData.error || "Erro ao renovar"}. Reautentique o app no Bling.`,
        },
      );
      ...
      return Response.json(
        {
          success: false,
          error: `${tokenData.error_description || tokenData.error || "Erro ao renovar token"}. Reautentique o app no Bling.`,
        },
