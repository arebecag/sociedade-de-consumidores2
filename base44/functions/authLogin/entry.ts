...
const JWT_SECRET = new TextEncoder().encode(
  Deno.env.get("INTERNAL_SECRET") || "sc3x3-secret-key-2024",
);
...
Deno.serve(async (req) => {
  try {
      ...
      try {
        ...
        await Promise.all(
          previousCodes.map((item) =>
            base44.asServiceRole.entities.EmailVerificationCode.update(
              item.id,
              {
                used: true,
              },
        ...
        console.error(
          "[authLogin] Erro ao enviar código de verificação:",
          e.message,
        );
      }
    }
    ...
    try {
      if (user.partner_id) {
        partner = await base44.asServiceRole.entities.Partner.get(
          user.partner_id,
        );
      }
      if (!partner) {
        const partnersByEmail =
          await base44.asServiceRole.entities.Partner.filter({
            email: emailNormalized,
          });
